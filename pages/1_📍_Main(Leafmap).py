    ...
    if mode=='Choropleth Map(Heritage Count)':
       ...
       heatmap(heritage2,m,"LATITUDE","LONGITUDE","AREAHA")
        ...
        with col3:
            ...
            line_charts = alt.Chart(years).mark_line(color='blue').encode(x=alt.X("DATEINSCRI",type='temporal'),y=alt.Y("count",type="quantitative"))
            point_charts=alt.Chart(years).mark_point().encode(x=alt.X("DATEINSCRI", type='temporal'),y=alt.Y("count", type="quantitative"),color=cond)
