    ...
    if mode=='Choropleth Map(Heritage Count)':
       ...
       heatmap(heritage2,m,"LATITUDE","LONGITUDE","AREAHA")
        ...
        with col3:
            ...
            line_charts = alt.Chart(years).mark_line(point=True).encode(x=alt.X("DATEINSCRI",type='temporal'),y=alt.Y("count",type="quantitative"),color=cond,tooltip=['DATEINSCRI','count'])
            point_charts=alt.Chart(years).mark_point(filled=True).encode(x=alt.X("DATEINSCRI", type='temporal'),y=alt.Y("count", type="quantitative"),color=cond)
