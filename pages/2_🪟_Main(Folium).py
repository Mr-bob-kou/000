        ...
        if "Region" in st.session_state.modes:
            ...
            if"Inscription Date" in st.session_state.modes:
                ...
                with col7:
                    ...
                    cond2=alt.condition(alt.datum.DATEINSCRI==Inscdate,alt.value('red'),alt.Color(alt.repeat("layer"),type='quantitative',scale=alt.Scale(domain=['C', 'C/N', 'N'], range=['blue', 'orange', 'green'])))
                    charts1 = alt.Chart(years).mark_line(point=True).encode(x=alt.X("DATEINSCRI",type='temporal'),y=alt.Y(alt.repeat("layer"),type='quantitative',title='count'), color=cond2).repeat(layer=["C", "C/N","N"])
