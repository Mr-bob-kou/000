        ...
        if "Region" in st.session_state.modes:
            ...
            if"Inscription Date" in st.session_state.modes:
                ...
                with col7:
                    ...
                    line_charts = alt.Chart(years).mark_line().encode(x=alt.X("DATEINSCRI",type='temporal'),y=alt.Y(alt.repeat("layer")), color=alt.datum(alt.repeat("layer"))).repeat(layer=["C", "C/N","N"])
                    ...
                    if Chart_mode=='Line Chart':
                        st.altair_chart(line_charts,use_container_width=True)
